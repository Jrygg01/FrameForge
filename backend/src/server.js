...
const maxOutputTokensEnv = Number.parseInt(process.env.OPENAI_RESPONSES_MAX_TOKENS ?? '3200', 10);
const maxOutputTokens = Number.isFinite(maxOutputTokensEnv) ? Math.max(256, maxOutputTokensEnv) : 3200;
...
app.post('/api/generate-ui', async (req, res) => {
  ...
  try {
    ...
    const promptSummary = prompt && prompt.trim().length ? prompt.trim() : 'Not provided';
    ...
    console.debug('generate-ui OpenAI response', JSON.stringify(response, null, 2));
    ...
    if (response.status !== 'completed') {
      ...
      throw new Error(`Model response incomplete (status: ${response.status}).`);
    }
    ...
    try {
      ...
      if (!extracted) {
        throw new Error(`Unable to parse model response as JSON. ${initialError.message}`);
      }
      try {
        parsed = JSON.parse(extracted);
      } catch (fallbackError) {
        throw new Error(`Unable to parse model response as JSON. ${fallbackError.message}`);
      }
    }
    ...
    if (typeof html !== 'string' || typeof css !== 'string' || typeof js !== 'string') {
      throw new Error('Model response missing expected html/css/js string fields.');
    }
